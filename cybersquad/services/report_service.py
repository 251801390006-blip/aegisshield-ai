"""
Cyber Squad AI – PDF Report Service
...
def generate_scan_report(scan_data: dict, user_data: dict) -> bytes:
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=2.5 * cm,
        bottomMargin=2 * cm,
        title=f"Cyber Squad AI – {scan_data.get('scan_type', 'Security').title()} Scan Report",
        author="Cyber Squad AI Platform",
...
def _build_header(styles, scan_data):
    ...
    elements.append(Paragraph("🛡 Cyber Squad AI", styles["ReportTitle"]))
...
def _build_footer_section(styles):
    elements = [
        ...
        Paragraph(
            "⚠ DISCLAIMER: This report is generated by Cyber Squad AI for informational purposes only. "
            "Always consult a qualified cybersecurity professional for critical decisions. "
            "Cyber Squad AI does not guarantee 100% detection accuracy.",
            styles["MetaText"],
        ),
        Paragraph(
            f"© {datetime.now().year} Cyber Squad AI Platform  |  Confidential Security Report",
            styles["MetaText"],
        ),
    ]
    return elements
...
def _add_watermark(canvas_obj, doc):
    ...
    canvas_obj.drawString(2 * cm, 0.28 * cm, "Cyber Squad AI – Confidential")
